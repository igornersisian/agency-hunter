...
def _call_provider(provider: str, messages: list, **kwargs):
...
def chat_completion(messages: list, max_retries: int = 3, **kwargs):
    ...
    for attempt in range(max_retries):
        try:
            return _call_provider(provider, messages, **dict(kwargs))
    ...
    # If primary provider exhausted retries, try the other one
    ...
    try:
        return _call_provider(fallback, messages, **dict(kwargs))
    except Exception:
        raise last_error
